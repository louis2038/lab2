...
void parallel_merge_sort(uint64_t *T, const uint64_t size, int depth) {
    ...
    #pragma omp task shared(T,depth) firstprivate(mid)
    {
        parallel_merge_sort(T, mid, depth + 1);
    }
    #pragma omp task shared(T,depth) firstprivate(mid)
    {
        parallel_merge_sort(T + mid, mid, depth + 1);
    }
    #pragma omp taskwait
